...
const CreateTradeLicence = ({ parentRoute }) => {
  ...
  const wizardConfig = useMemo(() => {
    let config = [];
    const nc = newConfig ? newConfig : newConfigTL;
    nc?.forEach((obj) => {
      config = config.concat(obj.body.filter((a) => !a.hideInCitizen));
    });
    let skipenanbledOb = nc?.filter((obj) => obj?.body?.some((com) => com.component === "CPTCreateProperty"))?.[0];
  ...
  const goNext = (skipStep, index, isAddMultiple, key, isPTCreateSkip) => {
    ...
    let { isCreateEnabled: enableCreate = true } = wizardConfig.find((routeObj) => routeObj.route === currentPath);
