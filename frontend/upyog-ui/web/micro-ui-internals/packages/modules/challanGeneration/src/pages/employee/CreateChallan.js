          ...
          if (result.challans && result.challans.length > 0) {
            ...
            Digit.ChallanGenerationService.generateBill(challan.challanNo, tenantId, challan.businessService, "challan").then((response) => {
              if (response.Bill && response.Bill.length > 0) {
                history.push(
                  `/upyog-ui/employee/challan-generation/acknowledgement?purpose=challan&status=success&tenantId=${tenantId}&billNumber=${
        ...
        .then((result, err) => {
          if (result.challans && result.challans.length > 0) {
            ...
            Digit.ChallanGenerationService.generateBill(challan.challanNo, tenantId, challan.businessService, "challan").then((response) => {
              if (response.Bill && response.Bill.length > 0) {
                history.push(
                  `/upyog-ui/employee/challan-generation/acknowledgement?purpose=challan&status=success&tenantId=${tenantId}&billNumber=${response.Bill[0].billNumber}&serviceCategory=${response.Bill[0].businessService}&challanNumber=${response.Bill[0].consumerCode}`,
                  { from: url }
                );
              }
            });
