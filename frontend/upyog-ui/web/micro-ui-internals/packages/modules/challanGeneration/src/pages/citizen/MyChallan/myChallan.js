...
import { Header, ResponseComposer, Card, KeyNote, SubmitBar } from "@upyog/digit-ui-react-components";
...
const MyChallanResult = ({ template, header, actionButtonLabel }) => {
  ...
  const handleMakePayment = (id) => {
    history.push(`/upyog-ui/citizen/payment/collect/Challan_Generation/${id}/${tenantId}?tenantId=${tenantId}`);
  };
          ...
          return (
            <Card key={index}>
              ...
              <div className="action-button-myapplication">
                {
                  <Link to={`/upyog-ui/citizen/challangeneration/application/${bill?.challanNo}/${bill?.tenantId}`}>
                    <SubmitBar
