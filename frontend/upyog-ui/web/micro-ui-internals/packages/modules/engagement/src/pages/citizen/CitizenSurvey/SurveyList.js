...
const SurveyList = () => {
  ...
  const handleCardClick = (details) => {
    ...
    if (!details.hasResponded) {
      navigate(`/upyog-ui/citizen/engagement/surveys/fill-survey?applicationNumber=${details?.code}&tenantId=${details?.tenantId}`, details);
    } else {
      navigate("/upyog-ui/citizen/engagement/surveys/show-survey", details);
    }
  };
